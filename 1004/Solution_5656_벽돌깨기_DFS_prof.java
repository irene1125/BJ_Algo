...
import java.io.*;
import java.util.*;
public class Solution_5656_벽돌깨기_DFS_prof {
	...
	private static void boomDFS(int[][] newMap, int r, int c, int cnt) {
		...
		for(int d = 0; d<4; d++) {
			int nx =r;
			int ny = c;
			for(int k = 1; k<cnt; k++) { // 해당 범위에 들어오는 블럭 crush
				nx +=dx[d];
				ny +=dy[d];
				if(nx>=0 && ny>=0 && nx<H && ny<W && newMap[nx][ny] > 0) {
					
					boomDFS(newMap, nx, ny, newMap[nx][ny]);
				}
			}
